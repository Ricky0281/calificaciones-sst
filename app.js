...
try {
  const evidencias = await getEvidencesByCedula(cedula);
  ...
  const payload = {
    ...
    fechaEnvio: nowISO(),
    contrato: "SIN_CONTRATO",
    cedula: cedula,
    nombre: empleado.nombre,
    centroCostos: empleado.centro,
    tipoEvidencia: "CALIFICACION_SST",
    observacion: "Registro enviado desde aplicativo Calificaciones SST",
    indicador: `${indicador}%`,
    estado: "Pendiente",
    archivos: archivos
  };

const respuesta = await fetch("https://25cb4771c5ebe819a4a4d073ad5cf5.09.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/0b6961def4674645975c44bbefb70ee4/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=JlGUm7hSXM93ecB6gqqypcLjzLcWkTqV_NH306tzac0", {
  method: "POST",
  headers: {
  ...
  if (!respuesta.ok) {
    throw new Error(`Error Power Automate: ${respuesta.status}`);
  }

  console.log("✅ Enviado correctamente a Power Automate");
